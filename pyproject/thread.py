...
Thread_total_num = 1  # 总进程
OneThread_num = 1  # 每个进程的并发数即是子进程
...
token=Login().login() # 可以直接调用函数获取token
# menber=member()
...
def work():  # 该函数支持添加单接口或者多接口
    ...
    order.payorder(token, trade_no)  # 余额购买
    order.qxzhifuorder(token, short_no)  # 取消已支付订单
